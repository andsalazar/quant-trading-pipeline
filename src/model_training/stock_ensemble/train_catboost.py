#!/usr/bin/env python3
"""
STEP 30c: TRAIN CATBOOST BASE MODEL (10-DAY HORIZON)
======================================================

Purpose:
- Train CatBoost per-symbol with walk-forward validation for 10-day predictions
...
Output:
- trained_models/catboost_10day/*.pkl (per-symbol models)
- catboost_predictions_validation_10day.csv (out-of-sample for meta-learner)
- catboost_training_log_10day.csv (performance metrics)
...
class CatBoostTrainer10Day:
    """Train CatBoost models per-symbol with walk-forward validation (10-day horizon)"""
    ...
    def __init__(self):
        ...
        self.model_dir = os.path.join(self.base_path, r'#_model_training\trained_models\catboost_10day')
        self.predictions_csv = os.path.join(self.base_path, r'#_model_training\catboost_predictions_validation_10day.csv')
        self.log_csv = os.path.join(self.base_path, r'#_model_training\catboost_training_log_10day.csv')
        
        # Create model directory
        os.makedirs(self.model_dir, exist_ok=True)
        
        print("=" * 80)
        print("STEP 30c: TRAIN CATBOOST BASE MODEL (10-DAY HORIZON)")
        print("=" * 80)
        ...
        if os.path.exists(optimized_params_file):
            ...
            with open(optimized_params_file, 'r') as f:
                ...
                cb_optimized = optimized_params['catboost']['params']
            
            # CatBoost optimized hyperparameters
            self.catboost_params = {
                'loss_function': 'Logloss',
                'eval_metric': 'AUC',
                'iterations': int(cb_optimized['iterations']),
                'learning_rate': cb_optimized['learning_rate'],
                'depth': int(cb_optimized['depth']),
                'l2_leaf_reg': cb_optimized['l2_leaf_reg'],
                'bagging_temperature': cb_optimized['bagging_temperature'],
                'random_strength': cb_optimized['random_strength'],
                'border_count': int(cb_optimized['border_count']),
                'random_seed': 42,
                'verbose': False,
                'early_stopping_rounds': 50,
                'task_type': 'CPU',  # CPU mode (faster for small per-symbol datasets)
                'thread_count': -1  # Use all CPU cores
            }
            print(f"  Improvement: {optimized_params['catboost']['improvement']}")
            print(f"  Test Accuracy: {optimized_params['catboost']['test_accuracy']:.4f}")
            print(f"  Test AUC: {optimized_params['catboost']['test_auc']:.4f}")
        else:
    ...
    def train_per_symbol(self, df, feature_cols):
        """Train CatBoost per symbol with walk-forward validation"""
        ...
        print(f"  (This may take 25-50 minutes)")
        ...
        for fold_idx, split in enumerate(self.validation_splits, 1):
            ...
            for sym_idx, symbol in enumerate(symbols, 1):
                try:
                    ...
                    # Train CatBoost (no scaler — trees are scale-invariant)
                    ...
                    fold_predictions.append(pd.DataFrame({
                        ...
                        'symbol': symbol,
                        'actual': y_test,
                        'catboost_pred_proba': y_pred_proba,
    ...
    def print_summary(self, training_log_df):
        ...
        print("\n" + "=" * 80)
        print("OK CatBoost training complete!")
        print("=" * 80)

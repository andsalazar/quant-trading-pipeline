#!/usr/bin/env python3
"""
STEP 30b: TRAIN LIGHTGBM BASE MODEL (10-DAY HORIZON)
=====================================================

Purpose:
- Train LightGBM per-symbol with walk-forward validation for 10-day predictions
- Generate out-of-sample predictions for 10-day meta-learner
...
- Expected: 56-58% AUC based on XGBoost 10-day results (55.5%)

Output:
- trained_models/lightgbm_10day/*.pkl (per-symbol models)
- lightgbm_predictions_validation_10day.csv (out-of-sample for meta-learner)
- lightgbm_training_log_10day.csv (performance metrics)
...
class LightGBMTrainer10Day:
    """Train LightGBM models per-symbol with walk-forward validation (10-day horizon)"""
    ...
    def __init__(self):
        ...
        self.model_dir = os.path.join(self.base_path, r'#_model_training\trained_models\lightgbm_10day')
        self.predictions_csv = os.path.join(self.base_path, r'#_model_training\lightgbm_predictions_validation_10day.csv')
        self.log_csv = os.path.join(self.base_path, r'#_model_training\lightgbm_training_log_10day.csv')
        
        # Create model directory
        os.makedirs(self.model_dir, exist_ok=True)
        
        print("=" * 80)
        print("STEP 30b: TRAIN LIGHTGBM BASE MODEL (10-DAY HORIZON)")
        print("=" * 80)
        ...
        print("Expected: 56-58% AUC (XGBoost 10-day achieved 55.5%)")
        print("=" * 80)
        ...
        if os.path.exists(optimized_params_file):
            ...
            with open(optimized_params_file, 'r') as f:
                ...
                lgb_optimized = optimized_params['lightgbm']['params']
            ...
            self.lgb_params = {
                ...
                'metric': 'auc',
                'boosting_type': 'gbdt',
                'num_leaves': int(lgb_optimized['num_leaves']),
                'learning_rate': lgb_optimized['learning_rate'],
                'n_estimators': int(lgb_optimized['n_estimators']),
                'min_child_samples': int(lgb_optimized['min_child_samples']),
                'subsample': lgb_optimized['subsample'],
                'subsample_freq': 5,
                'colsample_bytree': lgb_optimized['colsample_bytree'],
                'reg_alpha': lgb_optimized['reg_alpha'],
                'reg_lambda': lgb_optimized['reg_lambda'],
                'max_depth': int(lgb_optimized['max_depth']),
                'random_state': 42,
                'n_jobs': -1,
                'verbose': -1
            }
            print(f"  Improvement: {optimized_params['lightgbm']['improvement']}")
            print(f"  Test Accuracy: {optimized_params['lightgbm']['test_accuracy']:.4f}")
            print(f"  Test AUC: {optimized_params['lightgbm']['test_auc']:.4f}")
        else:
    ...
    def train_per_symbol(self, df, feature_cols):
        """Train LightGBM per symbol with walk-forward validation"""
        ...
        print(f"  (This may take 20-40 minutes - LightGBM is faster than XGBoost)")
        ...
        for fold_idx, split in enumerate(self.validation_splits, 1):
            ...
            for sym_idx, symbol in enumerate(symbols, 1):
                try:
                    ...
                    # Train LightGBM (no scaler — trees are scale-invariant)
                    ...
                    fold_predictions.append(pd.DataFrame({
                        ...
                        'symbol': symbol,
                        'actual': y_test,
                        'lightgbm_pred_proba': y_pred_proba,
    ...
    def print_summary(self, training_log_df):
        ...
        # Overall metrics
        print(f"  Overall AUC: {training_log_df['auc'].mean():.4f} (±{training_log_df['auc'].std():.4f})")
        print(f"  Overall Accuracy: {training_log_df['accuracy'].mean():.4f} (±{training_log_df['accuracy'].std():.4f})")
